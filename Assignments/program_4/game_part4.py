...
class Player(object):
  ...
  """
  def __init__(self,name=None,rolls=6,plan=None):
    ...
    self.strategy = plan                    # strategy name if any
  ...
  def Roll(self):
    if self.score < 100:
      ...
      if self.strategy == 'Cautious':
        self.MaxRolls -= 2
        self.RandomRoll()
      if self.strategy == 'Sprint to the Finish':
        if self.score >= 75:
          while not self.score >= 100:
            self.RandomRoll()
    else:
...
players = [Player('Bob',4,'Aggressive'),Player('Sue',4,'Conservative'),Player('Dax',4, 'Conservative'),Player('Ann',4, 'Sprint to the Finish')]
